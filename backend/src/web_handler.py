...
from process.TagProcess import handle_get_tags, handle_delete_tag, handle_get_tag_by_id, handle_update_tag, \
    handle_create_tag
from utils.HandleGammaToken import get_user_name_from_session, get_authorities_from_session
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        auth_res = get_authorities_from_session(session)
        if auth_res.is_error or (config.GAMMA_ADMIN_AUTHORITY not in auth_res.data):
...
def login_required(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        user_res = get_user_name_from_session(session)
...
class SongRes(Resource):
    def get(self, song_id):
        resp = handle_get_song_by_id(session, song_id).get_response()
        return resp
...
class SongsRes(Resource):
    def get(self):
        return handle_get_songs_and_tags(session).get_response()
...
class GammaSignout(Resource):
    def post(self):
        return handle_gamma_signout(session).get_response()
