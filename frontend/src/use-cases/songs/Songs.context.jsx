...
export const useSongTag = () => {
    const [{songs, tags, loadingSongs}, dispatch] = useContext(SongTagContext)

    const loadSongs = () => {
        if (!loadingSongs) {
            dispatch({type: SongTagActions.LOADING})
            getSongs().then(res => {
                const songs = Object.values(res.data.data.songs).sort((a, b) => a.number > b.number ? 1 : -1)
                const tags = Object.values(res.data.data.tags).sort((a, b) => a.name > b.name ? 1 : -1)
                dispatch({
